...
def XR_resave_zarr_wrapper(dataPaths, **kwargs):
    function_name = "XR_resave_zarr_wrapper"
    XR_resave_zarr_wrapper_dict = {
        "resultDirName": [kwargs.get("resultDirName", "resaved_zarr"), "char"],
        ...
        "channelPatterns": [kwargs.get("channelPatterns", ['.zarr']), "cell"],
        "inputBbox": [kwargs.get("inputBbox", []), "numericArr"],
        "parseCluster": [kwargs.get("parseCluster", False), "logical"],
        "largeFile": [kwargs.get("largeFile", True), "logical"],
    ...
    for key, value in XR_resave_zarr_wrapper_dict.items():
        if value[1] == "char":
            if not value[0]:
                continue
            cmdArgs += [key, value[0]]
